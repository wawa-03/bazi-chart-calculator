        ...
        <div className={`manual-reading${showThemes ? "" : " is-themes-paused"}`} id="manual-reading">
          {!started ? null : (
            <>
              ...
              {activeEntry && activeMonth !== null && <article className="focus-reading-card" aria-live="polite"><div className="focus-reading-meta"><span>{copy.first} / {profile.year}</span><b>{manualMonth(selectedLocale, activeMonth)}</b></div><h4>{activeEntry.title}</h4><p className="focus-personal-line">{selectedLocale === "en" ? <>Your <strong>{dayPillar}</strong> pillar and the next solar term are a reference.</> : <>日柱和下一节气，只作参考。</>}</p><section><span>{copy.cue}</span><p>{activeEntry.focus}</p></section><section><span>{copy.question}</span><p>{activeEntry.prompt}</p></section><section><span>{copy.action}</span><p>{activeEntry.note}</p></section><p className="monthly-disclaimer">{selectedLocale === "en" ? "Use it as a prompt, not a forecast." : selectedLocale === "zh-TW" ? "只作參考，不預測結果。" : "只作参考，不预测结果。"}</p></article>}
