...
const annualUiCopy = {
  "zh-CN": { kicker: "年度命书", intro: "不用一次看完。", accent: "先看下一卷。", body: "选年份，马上开始。", name: "怎么称呼你", namePlaceholder: "例如：王二小", year: "看哪一年", birthPlace: "出生地（可选）", birthPlaceholder: "例如：北京", residence: "现住地址（可选）", residencePlaceholder: "保存前不会记录", privacy: "不保存，就不会留下。", emptyKicker: "下一步", emptyTitle: "先选称呼和年份。", emptyBody: "我们会打开下一卷未来月卷。", unnamed: "未署名" },
  "zh-TW": { kicker: "年度命書", intro: "不用一次看完。", accent: "先看下一卷。", body: "選年份，馬上開始。", name: "怎麼稱呼你", namePlaceholder: "例如：王二小", year: "看哪一年", birthPlace: "出生地（可選）", birthPlaceholder: "例如：北京", residence: "現住地址（可選）", residencePlaceholder: "保存前不會記錄", privacy: "不保存，就不會留下。", emptyKicker: "下一步", emptyTitle: "先選稱呼和年份。", emptyBody: "我們會打開下一卷未來月卷。", unnamed: "未署名" },
  en: { kicker: "ANNUAL", intro: "No need to read it all.", accent: "Start with the next volume.", body: "Choose a year. Start now.", name: "What should we call you?", namePlaceholder: "For example: Wang Erxiao", year: "Which year?", birthPlace: "Birthplace (optional)", birthPlaceholder: "For example: Beijing", residence: "Address (optional)", residencePlaceholder: "Not saved unless you save", privacy: "Not saved, not kept.", emptyKicker: "NEXT STEP", emptyTitle: "Choose a name and year.", emptyBody: "We will open your next future volume.", unnamed: "Unnamed" },
} as const;
    ...
    <section className="manual-section" id="manual" aria-labelledby="manual-title">
      ...
      <div className="manual-layout">
        ...
        <div className="manual-reading" id="manual-reading">
          {!started ? (
            <div className="manual-empty-state focus-empty-state"><Compass /><div><span>{ui.emptyKicker}</span><h3>{ui.emptyTitle}</h3><p>{ui.emptyBody}</p><p className="theme-entry-note"><NotebookPen /> {selectedLocale === "en" ? "Then explore relationships, work, money, or daily rhythm." : "开始后，可看关系、事业、财务和生活节奏。"}</p></div></div>
          ) : (
            <>
              ...
              {activeEntry && activeMonth !== null && <article className="focus-reading-card" aria-live="polite"><div className="focus-reading-meta"><span>{copy.first} / {profile.year}</span><b>{manualMonth(selectedLocale, activeMonth)}</b></div><h4>{activeEntry.title}</h4><p className="focus-personal-line">{selectedLocale === "en" ? <>Your <strong>{dayPillar}</strong> day pillar and the next solar term <strong>{annualAccess?.nextJie || ""}</strong> guide this page.</> : <>用你的<strong>{dayPillar}</strong>日柱和下一节<strong>{annualAccess?.nextJie || ""}</strong>做线索。</>}</p><section><span>{copy.cue}</span><p>{activeEntry.focus}</p></section><section><span>{copy.question}</span><p>{activeEntry.prompt}</p></section><section><span>{copy.action}</span><p>{activeEntry.note}</p></section><p className="monthly-disclaimer">{selectedLocale === "en" ? "A prompt for reflection, not a prediction." : "只作参考，不预测结果。"}</p></article>}
