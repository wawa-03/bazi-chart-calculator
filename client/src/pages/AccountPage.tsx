  ...
  return (
    <ProductPage>
      ...
      <section className="account-grid">
        ...
        <article className="account-card"><header><div><span>HUMAN READING</span><h2>人工深度解读申请</h2></div><Link href="/consultation">提交申请 <ChevronRight /></Link></header>{consultations.isLoading ? <p>正在读取…</p> : consultations.data?.length ? <ul className="account-list consultation-list">{consultations.data.map((item) => <li key={item.id}><ScrollText /><div><b>{serviceCopy[item.service]}</b><small>更新于 {new Date(item.updatedAt).toLocaleString("zh-CN")} · <em className={`status-${item.status}`}>{statusCopy[item.status]}</em></small><div className="request-progress" aria-label={`当前进度：${statusCopy[item.status]}`}>{progressStages.map((stage) => <i key={stage} className={progressStages.indexOf(stage) <= progressStages.indexOf(item.status as typeof progressStages[number]) ? "is-done" : ""} title={statusCopy[stage]} />)}</div></div><button type="button" onClick={() => removeConsultation.mutate({ id: item.id })} disabled={removeConsultation.isPending} aria-label="删除申请"><Trash2 /></button></li>)}</ul> : <div className="account-empty"><ScrollText /><p>尚未提交咨询申请。服务方会在你选择的渠道联系你安排后续。</p><Link href="/consultation">申请人工解读 <ChevronRight /></Link></div>}<p className="request-refresh-note">进度每 20 秒自动刷新；如有新安排，会显示最新更新时间。</p></article>
        <article className="account-card"><header><div><span>PRIVATE REFLECTIONS</span><h2>主题回顾笔记</h2></div><Link href="/chart#manual">回到命书 <ChevronRight /></Link></header>{notes.isLoading ? <p>正在读取…</p> : notes.data?.length ? <><div className="note-filters"><label><Search /><input value={noteQuery} onChange={(event) => setNoteQuery(event.target.value)} placeholder="搜索笔记关键词" /></label><label><Filter /><select value={noteYear} onChange={(event) => setNoteYear(event.target.value)}><option value="all">全部年份</option>{archiveYears.map((year) => <option key={year} value={year}>{year} 年</option>)}</select></label></div>{filteredNotes.length ? <ul className="account-list">{filteredNotes.map((item) => <li key={item.id}><NotebookPen /><div><b>{themeCopy[item.themeKey as keyof typeof themeCopy] || "主题回顾"}</b><small>{archiveYearsById.get(item.archiveId) || "—"} 年 · {item.content.slice(0, 72)}{item.content.length > 72 ? "…" : ""}</small></div><Link href="/chart#manual">查看 <ChevronRight /></Link></li>)}</ul> : <div className="account-empty"><Search /><p>没有符合当前关键词或年份的私有回顾。</p></div>}</> : <div className="account-empty"><NotebookPen /><p>还没有主题回顾笔记。进入年度命书的主题卡片后，可为已保存命书写下仅自己可见的回顾。</p><Link href="/chart#manual">进入主题阅读 <ChevronRight /></Link></div>}</article>
      </section>
