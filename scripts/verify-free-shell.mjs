...
try {
  ...
  const fortuneOverview = desktop.locator(".result-fortune-overview");
  assert.equal(await fortuneOverview.isVisible(), true);
  assert.match(await fortuneOverview.innerText(), /当前大运|CURRENT DA YUN/);
  assert.match(await fortuneOverview.innerText(), /阅读索引|reading index|fixed verdict/i);
  ...
  assert.match(await financeDirection.innerText(), /财务与资源|MONEY & RESOURCES/);
  ...
  assert.match(await financeDirection.innerText(), /不构成投资|Not investment/);
  ...
  await mobile.waitForSelector(".result-fortune-overview", { state: "visible" });
  assert.match(await mobile.locator(".result-fortune-overview").innerText(), /当前大运|CURRENT DA YUN/);
  assert.equal(await mobile.locator(".result-fortune-overview .fortune-direction-card").count(), 4);
